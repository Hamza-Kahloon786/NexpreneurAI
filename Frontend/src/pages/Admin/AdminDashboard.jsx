  ...
  return (
    <div style={{
      ...
      <div style={{ minWidth: 0 }}>
        <p style={{ fontSize: 12.5, color: colors.muted, margin: '0 0 4px', fontWeight: 500 }}>{label}</p>
        <p style={{ fontSize: 28, fontWeight: 700, color: colors.dark, margin: 0, lineHeight: 1 }}>{value ?? '—'}</p>
        {sub && <p style={{ fontSize: 11.5, color: colors.muted, margin: '4px 0 0' }}>{sub}</p>}
      </div>
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  const gridCols = winW < 560 ? '1fr' : winW < 900 ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)';
    ...
    <AdminLayout>
      <div style={{ maxWidth: 1000 }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: isMobile ? 22 : 26, fontWeight: 700, color: colors.dark, margin: '0 0 6px', letterSpacing: '-0.02em' }}>
            Dashboard
          </h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: gridCols, gap: 14, marginBottom: 24 }}>
          <StatCard label="Total Users"    value={stats?.total}     sub="All registered accounts"       accent={colors.purple}
          ...
          />
          <StatCard label="Active Users"   value={stats?.active}    sub="Accounts currently enabled"    accent="#16a34a"
          ...
          />
          <StatCard label="Inactive Users" value={stats?.inactive}  sub="Accounts currently disabled"   accent="#dc2626"
          ...
          />
          <StatCard label="New This Week"  value={stats?.newThisWeek}  sub="Registered in the last 7 days"     accent="#d97706"
          ...
          />
          <StatCard label="New This Month" value={stats?.newThisMonth} sub="Registered this calendar month"    accent="#0891b2"
          ...
          />
          <StatCard label="Active Rate"    value={stats ? `${Math.round((stats.active / (stats.total || 1)) * 100)}%` : null} sub="Percentage of active accounts" accent={colors.purpleMid}
